...
import { LayoutGrid, List, Pin, Lock, X, Menu, Upload, Search, Plus, Tag, Share2, Trash2, LogOut, User, ChevronRight, Bell, KeyRound, WifiOff } from 'lucide-react';
...
import {
    saveNotesToLocal, getNotesFromLocal,
    saveLabelsToLocal, getLabelsFromLocal,
    addPendingAction, getPendingActions, deletePendingAction,
    isOnline,
...
export default function Home() {
    ...
    // ── Sync pending actions khi có mạng lại (dùng useCallback để useEffect gọi được) ──
    const syncPendingActions = useCallback(async () => {
        const pending = await getPendingActions();
        if (pending.length === 0) return;
        for (const action of pending) {
            try {
                if (action.method === 'PUT') await axios.put(action.url, action.data);
                else if (action.method === 'POST') await axios.post(action.url, action.data);
                else if (action.method === 'DELETE') await axios.delete(action.url);
    ...
    // ── Fetch notes: online → API + cache, offline → IndexedDB ──
    const fetchNotes = useCallback(async () => {
        if (!isOnline()) {
            const local = await getNotesFromLocal();
            setNotes(local);
            setLoading(false);
            return;
        }
        try {
            ...
            setNotes(res.data);
            await saveNotesToLocal(res.data);
        } catch (err) {
            const local = await getNotesFromLocal();
    ...
    // ── Fetch labels: online → API + cache, offline → IndexedDB ──
    const fetchLabels = useCallback(async () => {
        if (!isOnline()) {
            const local = await getLabelsFromLocal();
            setLabels(local);
            return;
        }
        try {
            ...
            setLabels(res.data);
            await saveLabelsToLocal(res.data);
        } catch (err) {
            const local = await getLabelsFromLocal();
            if (local.length > 0) setLabels(local);
        }
    }, []);

    useEffect(() => { fetchNotes(); fetchLabels(); fetchUnseenCount(); }, []);
    ...
    useEffect(() => {
        ...
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker.addEventListener('message', (e) => {
                if (e.data?.type === 'SYNC_PENDING') syncPendingActions();
            });
        }
    ...
    }, [syncPendingActions, fetchNotes, fetchLabels]);
    ...
    const fetchUnseenCount = async () => {
        try {
            const res = await axios.get('/api/shares/unseen-count');
            setUnseenCount(res.data.count);
        } catch (err) { /* bỏ qua lỗi offline */ }
    };

    const fetchSharedWithMe = async () => {
        const res = await axios.get('/api/shared-with-me');
        setSharedWithMe(res.data);
        await axios.post('/api/shares/mark-all-seen');
        setUnseenCount(0);
    };
    ...
    const saveNote = async () => {
        ...
        try {
            if (!isOnline()) {
                if (editNote) {
                    await addPendingAction({ method: 'PUT', url: `/api/notes/${editNote.id}`, data: form });
                    const local = await getNotesFromLocal();
                    const updated = local.map(n =>
                        n.id === editNote.id ? { ...n, ...form, updated_at: new Date().toISOString() } : n
                    );
                    await saveNotesToLocal(updated);
                    setNotes(updated);
                } else {
                    const tempNote = {
                        id: -Date.now(), ...form, is_pinned: 0,
                        created_at: new Date().toISOString(),
                        updated_at: new Date().toISOString(),
                        labels: [], images: [],
                    };
                    await addPendingAction({ method: 'POST', url: '/api/notes', data: form, tempId: tempNote.id });
                    const local = await getNotesFromLocal();
                    await saveNotesToLocal([tempNote, ...local]);
    ...
    const confirmDelete = async (id) => {
        ...
        if (!isOnline()) {
            await addPendingAction({ method: 'DELETE', url: `/api/notes/${id}` });
            const local = await getNotesFromLocal();
    ...
    const handleLabelsClose = async () => {
        ...
        if (editNote) {
            ...
            if (updated) {
                setEditNote(updated);
                setNoteLabels(updated.labels ? updated.labels.map(l => l.id) : []);
            }
            setNotes(res.data);
        } else {
            fetchNotes();
            ...
            <div className="p-4 flex-1 flex flex-col">
                <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                        ...
                        {note.shares && note.shares.length > 0 && <Share2 size={12} className="text-indigo-400" />}
                ...
                <div className={`flex items-center gap-1 pt-2 border-t opacity-0 group-hover:opacity-100 transition-opacity
                    ...
                    {!note.password && (
                        <button onClick={(e) => { e.stopPropagation(); setPasswordModal({ note, mode: 'set' }); }}
                    ...
                    {note.password && (
                        <>
                            <button onClick={(e) => {
                                e.stopPropagation();
                                setPasswordModal({ note, mode: 'verify', afterVerify: () => setPasswordModal({ note, mode: 'change' }) });
                            }}
                                className={`flex items-center gap-1 text-xs px-2 py-1 rounded-lg transition
                                    ${isDark ? 'hover:bg-amber-900/30 text-slate-400 hover:text-amber-400' : 'hover:bg-amber-50 text-slate-400 hover:text-amber-600'}`}>
                                <KeyRound size={10} />Đổi mk
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); setPasswordModal({ note, mode: 'remove' }); }}
                                className={`flex items-center gap-1 text-xs px-2 py-1 rounded-lg transition
                                    ${isDark ? 'hover:bg-rose-900/30 text-slate-400 hover:text-rose-400' : 'hover:bg-rose-50 text-slate-400 hover:text-rose-500'}`}>
                                <Lock size={10} />Tắt khóa
                            </button>
                        </>
                    )}
    ...
    return (
        <div className={`min-h-screen font-sans ${isDark ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
            ...
            {/* OFFLINE BANNER — đặt trước header, dùng sticky thay fixed để không che header */}
            {offlineMode && (
                <div className="sticky top-0 z-50 bg-amber-500 text-white text-center text-sm py-1.5 flex items-center justify-center gap-2">
                    <WifiOff size={14} />
                    <span>Bạn đang offline — Dữ liệu lấy từ bộ nhớ cục bộ, thay đổi sẽ tự đồng bộ khi có mạng</span>
                </div>
            )}
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-6 h-14 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => { setShowSharedWithMe(true); fetchSharedWithMe(); }}
                            className={`relative flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-xl transition
                                ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-slate-300' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}>
                            <Bell size={14} />
                            <span className="hidden sm:inline">Chia sẻ với tôi</span>
                            {unseenCount > 0 && (
            ...
            <div className="flex">
                <aside className={`w-52 flex-shrink-0 min-h-[calc(100vh-3.5rem)] border-r hidden md:block sticky top-14 self-start
                                                ...
                                                {editNote.images.map(img => (
                                                    <div key={img.id} className="relative group/img">
                                                        ...
                                                        <button onClick={() => deleteImage(img.id)}
                                                            className="absolute -top-1.5 -right-1.5 bg-rose-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs opacity-0 group-hover/img:opacity-100 transition">
                                                            ×
                                                        </button>
                                                    </div>
                                                ))}
            ...
            {showLabels && <Labels onClose={handleLabelsClose} />}
                                        ...
                                        <li key={item.share_id}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {!item.is_seen && (
                                                        <span className="text-[10px] bg-rose-500 text-white px-1.5 py-0.5 rounded-full font-bold flex-shrink-0">Mới</span>
                                                    )}
                                                </div>
